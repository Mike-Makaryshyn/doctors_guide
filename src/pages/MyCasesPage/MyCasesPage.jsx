...
const MyCasesPage = () => {
  ...
  const handleEdit = (myCase) => {
    // Навігація до сторінки редагування з передачею даних випадку
 ...
 <div className={styles.container}>
      <h1>Мої Випадки</h1>
      {myCases.length === 0 ? (
        <p>У вас немає збережених випадків.</p>
      ) : (
        <table className={styles.casesTable}>
          <thead>
            <tr>
              {/* Зміна заголовка з ID на FullName */}
              <th>FullName</th>
              <th>Тема</th>
              <th>Регіон</th>
              <th>Дії</th>
          ...
          <tbody>
            {myCases.map((myCase) => (
              <tr key={myCase.id}>
                {/* Відображення FullName замість ID */}
                <td>{myCase.fullName}</td>
                <td>{myCase.theme}</td>
                <td>{myCase.region}</td>
                <td>
                  <button
