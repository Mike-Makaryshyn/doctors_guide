const tutorialTranslations = {
  currentLanguage: "en", // Значення за замовчуванням
  en: {
    steps: {
      intro:
        "Welcome to Elective Language Game – Medications Edition! In this game, you'll practice medication names in different languages. <strong>If you answer correctly 5 times for a medication, it will automatically be marked as learned.</strong> This game is designed to help you prepare for your Fachsprachenprüfung. Enjoy the game!",
      regionSelect:
        "The region is set based on your preferences; however, you can change it here.",
      filterColumn:
        "Here you can choose the filter: learned, unlearned, or paused (deferred).",
      categorySelect:
        "Choose the category of medications. By default, 'All' is selected.",
      editToggleButton:
        "When edit mode is activated, you can modify your answer after an incorrect response.",
      languageSwapContainer:
        "Use this container to swap the translation direction. Click the button to exchange source and target languages.",
      questionCount:
        "Select the number of questions to be displayed in the game.",
      startButton:
        "Press Start to begin the game."
    },
    buttons: {
      back: "Back",
      close: "Close",
      last: "Finish",
      next: "Next",
      skip: "Skip"
    }
  },
  de: {
    steps: {
      intro:
        "Willkommen beim Elective Language Game – Medications Edition! In diesem Spiel üben Sie die Namen von Medikamenten in verschiedenen Sprachen. <strong>Wenn Sie einen Medikamentennamen 5 Mal korrekt beantworten, wird er automatisch als gelernt markiert.</strong> Dieses Spiel unterstützt Sie optimal bei der Vorbereitung auf die Fachsprachenprüfung. Viel Spaß beim Spielen!",
      regionSelect:
        "Die Region wird basierend auf Ihren Einstellungen festgelegt; Sie können sie jedoch hier ändern.",
      filterColumn:
        "Hier können Sie den Filter auswählen: Gelernt, Ungelernt oder Pausiert.",
      categorySelect:
        "Wählen Sie die Kategorie der Medikamente. Standardmäßig ist 'Alle' ausgewählt.",
      editToggleButton:
        "Wenn der Bearbeitungsmodus aktiviert ist, können Sie Ihre Antwort nach einer falschen Antwort ändern.",
      languageSwapContainer:
        "Verwenden Sie diesen Container, um die Übersetzungsrichtung zu wechseln. Klicken Sie auf den Button, um Quell- und Zielsprache auszutauschen.",
      questionCount:
        "Wählen Sie die Anzahl der Fragen, die im Spiel angezeigt werden sollen.",
      startButton:
        "Drücken Sie auf Start, um das Spiel zu beginnen."
    },
    buttons: {
      back: "Zurück",
      close: "Schließen",
      last: "Fertigstellen",
      next: "Weiter",
      skip: "Überspringen"
    }
  },
  uk: {
    steps: {
      intro:
        "Ласкаво просимо до Elective Language Game – Medications Edition! У цій грі вам будуть представлені назви ліків для практики різними мовами. <strong>Якщо ви відповісте правильно 5 разів на один ліковий термін, він автоматично буде позначений як вивчений.</strong> Гра допомагає вам підготуватися до Fachsprachenprüfung. Бажаємо успіхів!",
      regionSelect:
        "Регіон встановлено за вашими налаштуваннями, але його можна змінити тут.",
      filterColumn:
        "Тут ви можете обрати фільтр: вивчені, невивчені або призупинені.",
      categorySelect:
        "Виберіть категорію ліків. За замовчуванням обрано «Всі».",
      editToggleButton:
        "При активованому режимі редагування ви зможете змінити свою відповідь після неправильної відповіді.",
      languageSwapContainer:
        "Використовуйте цей контейнер для зміни напрямку перекладу. Натисніть кнопку, щоб обміняти місцями джерело та ціль.",
      questionCount:
        "Виберіть кількість питань, які будуть відображатися в грі.",
      startButton:
        "Натисніть Старт, щоб розпочати гру."
    },
    buttons: {
      back: "Назад",
      close: "Закрити",
      last: "Завершити",
      next: "Далі",
      skip: "Пропустити"
    }
  },
  ru: {
    steps: {
      intro:
        "Добро пожаловать в Elective Language Game – Medications Edition! В этой игре вы будете практиковаться с названиями лекарств на разных языках. <strong>Если вы правильно ответите 5 раз на одно лекарство, оно автоматически будет отмечено как изученное.</strong> Игра поможет вам подготовиться к Fachsprachenprüfung. Приятной игры!",
      regionSelect:
        "Регион устанавливается на основе ваших настроек, однако вы можете изменить его здесь.",
      filterColumn:
        "Здесь вы можете выбрать фильтр: изученные, не изученные или отложенные.",
      categorySelect:
        "Выберите категорию лекарств. По умолчанию выбраны все.",
      editToggleButton:
        "При активированном режиме редактирования вы сможете изменить свой ответ после неверного ответа.",
      languageSwapContainer:
        "Используйте этот контейнер для смены направления перевода. Нажмите кнопку для обмена исходного и целевого языка.",
      questionCount:
        "Выберите количество вопросов, которые будут отображаться в игре.",
      startButton:
        "Нажмите Старт, чтобы начать игру."
    },
    buttons: {
      back: "Назад",
      close: "Закрыть",
      last: "Завершить",
      next: "Далее",
      skip: "Пропустить"
    }
  },
  pl: {
    steps: {
      intro:
        "Witamy w Elective Language Game – Medications Edition! W tej grze będziesz ćwiczyć nazwy leków w różnych językach. <strong>Jeśli odpowiesz poprawnie 5 razy na dany lek, zostanie on automatycznie oznaczony jako nauczony.</strong> Gra ma na celu pomóc Ci przygotować się do egzaminu Fachsprachenprüfung. Miłej zabawy!",
      regionSelect:
        "Region jest ustawiony na podstawie Twoich preferencji; jednak możesz go zmienić tutaj.",
      filterColumn:
        "Tutaj możesz wybrać filtr: nauczone, nienauczone lub wstrzymane.",
      categorySelect:
        "Wybierz kategorię leków. Domyślnie wybrane są wszystkie.",
      editToggleButton:
        "Gdy tryb edycji jest aktywny, możesz zmienić swoją odpowiedź po błędnej odpowiedzi.",
      languageSwapContainer:
        "Użyj tego kontenera, aby zmienić kierunek tłumaczenia. Kliknij przycisk, aby zamienić miejscami język źródłowy i docelowy.",
      questionCount:
        "Wybierz liczbę pytań, które mają być wyświetlane w grze.",
      startButton:
        "Naciśnij Start, aby rozpocząć grę."
    },
    buttons: {
      back: "Wstecz",
      close: "Zamknij",
      last: "Zakończ",
      next: "Dalej",
      skip: "Pomiń"
    }
  },
  ar: {
    steps: {
      intro:
        "مرحبًا بك في Elective Language Game – Medications Edition! في هذه اللعبة ستتدرب على أسماء الأدوية بعدة لغات. <strong>إذا أجبت بشكل صحيح 5 مرات على دواء معين، فسيتم تمييزه تلقائيًا على أنه متعلم.</strong> تم تصميم هذه اللعبة لمساعدتك في الاستعداد لامتحان Fachsprachenprüfung. استمتع باللعبة!",
      regionSelect:
        "يتم تعيين المنطقة بناءً على إعداداتك؛ ومع ذلك، يمكنك تغييرها هنا.",
      filterColumn:
        "هنا يمكنك اختيار الفلتر: المُتعلمة، غير المُتعلمة أو المؤجلة.",
      categorySelect:
        "اختر فئة الأدوية. بشكل افتراضي، يتم اختيار الكل.",
      editToggleButton:
        "عند تفعيل وضع التعديل، يمكنك تغيير إجابتك بعد الإجابة الخاطئة.",
      languageSwapContainer:
        "استخدم هذا الحاوية لتبديل اتجاه الترجمة. انقر على الزر لتبديل اللغة المصدر والهدف.",
      questionCount:
        "اختر عدد الأسئلة التي ستظهر في اللعبة.",
      startButton:
        "اضغط على بدء لبدء اللعبة."
    },
    buttons: {
      back: "رجوع",
      close: "إغلاق",
      last: "إنهاء",
      next: "التالي",
      skip: "تخطي"
    }
  },
  tr: {
    steps: {
      intro:
        "Elective Language Game – Medications Edition'a hoş geldiniz! Bu oyunda, farklı dillerde ilaç isimleri üzerinde pratik yapacaksınız. <strong>Bir ilaca 5 defa doğru cevap verirseniz, o otomatik olarak öğrenilmiş olarak işaretlenecektir.</strong> Bu oyun, Fachsprachenprüfung'e hazırlık sürecinizde size yardımcı olmak amacıyla tasarlanmıştır. İyi eğlenceler!",
      regionSelect:
        "Bölge, tercihleriniz doğrultusunda ayarlanır; ancak buradan değiştirebilirsiniz.",
      filterColumn:
        "Burada, öğrenilmiş, öğrenilmemiş veya askıya alınmış filtreyi seçebilirsiniz.",
      categorySelect:
        "İlaç kategorisini seçin. Varsayılan olarak 'Hepsi' seçilidir.",
      editToggleButton:
        "Düzenleme modu etkinleştirildiğinde, yanlış cevap verdikten sonra cevabınızı değiştirebilirsiniz.",
      languageSwapContainer:
        "Bu konteyner, çeviri yönünü değiştirmek içindir. Butona basarak kaynak ve hedef diller yer değiştirir.",
      questionCount:
        "Oyunda gösterilecek soru sayısını seçin.",
      startButton:
        "Oyunu başlatmak için Başlat'a basın."
    },
    buttons: {
      back: "Geri",
      close: "Kapat",
      last: "Bitir",
      next: "İleri",
      skip: "Atla"
    }
  },
  fr: {
    steps: {
      intro:
        "Bienvenue dans Elective Language Game – Medications Edition ! Dans ce jeu, vous pratiquerez les noms des médicaments dans différentes langues. <strong>Si vous répondez correctement 5 fois à un médicament, il sera automatiquement marqué comme appris.</strong> Ce jeu est spécialement conçu pour vous aider à préparer votre Fachsprachenprüfung. Amusez-vous bien !",
      regionSelect:
        "La région est définie en fonction de vos préférences ; toutefois, vous pouvez la modifier ici.",
      filterColumn:
        "Ici, vous pouvez choisir le filtre : appris, non appris ou en pause.",
      categorySelect:
        "Choisissez la catégorie des médicaments. Par défaut, « Tous » est sélectionné.",
      editToggleButton:
        "Lorsque le mode édition est activé, vous pouvez modifier votre réponse après une erreur.",
      languageSwapContainer:
        "Utilisez ce conteneur pour inverser la direction de traduction. Appuyez sur le bouton pour échanger la langue source et la langue cible.",
      questionCount:
        "Sélectionnez le nombre de questions à afficher dans le jeu.",
      startButton:
        "Appuyez sur Démarrer pour commencer le jeu."
    },
    buttons: {
      back: "Retour",
      close: "Fermer",
      last: "Terminer",
      next: "Suivant",
      skip: "Passer"
    }
  },
  es: {
    steps: {
      intro:
        "¡Bienvenido a Elective Language Game – Medications Edition! En este juego, practicarás los nombres de los medicamentos en diferentes idiomas. <strong>Si respondes correctamente 5 veces a un medicamento, se marcará automáticamente como aprendido.</strong> Este juego está diseñado para ayudarte a prepararte para el Fachsprachenprüfung. ¡Disfruta del juego!",
      regionSelect:
        "La región se establece según tus preferencias; sin embargo, puedes cambiarla aquí.",
      filterColumn:
        "Aquí puedes elegir el filtro: aprendido, no aprendido o en pausa.",
      categorySelect:
        "Elige la categoría de los medicamentos. Por defecto, se selecciona 'Todos'.",
      editToggleButton:
        "Cuando se active el modo de edición, podrás modificar tu respuesta tras un error.",
      languageSwapContainer:
        "Utiliza este contenedor para cambiar la dirección de traducción. Pulsa el botón para intercambiar el idioma de origen y el de destino.",
      questionCount:
        "Selecciona la cantidad de preguntas que se mostrarán en el juego.",
      startButton:
        "Presiona Iniciar para comenzar el juego."
    },
    buttons: {
      back: "Atrás",
      close: "Cerrar",
      last: "Finalizar",
      next: "Siguiente",
      skip: "Omitir"
    }
  },
  el: {
    steps: {
      intro:
        "Καλώς ήρθατε στο Elective Language Game – Medications Edition! Σε αυτό το παιχνίδι, θα εξασκηθείτε στα ονόματα φαρμάκων σε διάφορες γλώσσες. <strong>Εάν απαντήσετε σωστά 5 φορές για ένα φάρμακο, θα επισημανθεί αυτόματα ως μάθημένο.</strong> Αυτό το παιχνίδι έχει σχεδιαστεί για να σας βοηθήσει να προετοιμαστείτε για την Fachsprachenprüfung. Καλή διασκέδαση!",
      regionSelect:
        "Η περιοχή καθορίζεται βάσει των προτιμήσεών σας· ωστόσο, μπορείτε να την αλλάξετε εδώ.",
      filterColumn:
        "Εδώ μπορείτε να επιλέξετε το φίλτρο: μάθημένα, αμαθημένα ή σε παύση (αναβλημένα).",
      categorySelect:
        "Επιλέξτε την κατηγορία των φαρμάκων. Από προεπιλογή, έχει επιλεγεί το 'Όλα'.",
      editToggleButton:
        "Όταν ενεργοποιείται η λειτουργία επεξεργασίας, μπορείτε να τροποποιήσετε την απάντησή σας μετά από λανθασμένη απάντηση.",
      languageSwapContainer:
        "Χρησιμοποιήστε αυτό το κοντέινερ για να αλλάξετε την κατεύθυνση της μετάφρασης. Κάντε κλικ στο κουμπί για να ανταλλάξετε τη γλώσσα πηγής με τη γλώσσα στόχο.",
      questionCount:
        "Επιλέξτε τον αριθμό των ερωτήσεων που θα εμφανίζονται στο παιχνίδι.",
      startButton:
        "Πατήστε το Start για να ξεκινήσετε το παιχνίδι."
    },
    buttons: {
      back: "Πίσω",
      close: "Κλείσιμο",
      last: "Τέλος",
      next: "Επόμενο",
      skip: "Παράβλεψη"
    }
  },
  ro: {
    steps: {
      intro:
        "Bine ați venit la Elective Language Game – Medications Edition! În acest joc, vă veți exersa numele medicamentelor în diferite limbi. <strong>Dacă răspundeți corect de 5 ori pentru un medicament, acesta va fi marcat automat ca învățat.</strong> Acest joc este conceput pentru a vă ajuta să vă pregătiți pentru examenul Fachsprachenprüfung. Bucurați-vă de joc!",
      regionSelect:
        "Regiunea este setată în funcție de preferințele dvs.; totuși, o puteți schimba aici.",
      filterColumn:
        "Aici puteți alege filtrul: învățate, neînvățate sau în pauză (amânate).",
      categorySelect:
        "Alegeți categoria medicamentelor. Implicit, este selectat 'Toate'.",
      editToggleButton:
        "Când modul de editare este activat, puteți modifica răspunsul după o greșeală.",
      languageSwapContainer:
        "Utilizați acest container pentru a schimba direcția traducerii. Apăsați butonul pentru a schimba limbile sursă și țintă.",
      questionCount:
        "Selectați numărul de întrebări care vor fi afișate în joc.",
      startButton:
        "Apăsați Start pentru a începe jocul."
    },
    buttons: {
      back: "Înapoi",
      close: "Închide",
      last: "Finalizare",
      next: "Următorul",
      skip: "Sari peste"
    }
  }
};

export default tutorialTranslations;