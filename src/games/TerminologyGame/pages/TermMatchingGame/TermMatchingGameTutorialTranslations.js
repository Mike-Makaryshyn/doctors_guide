const tutorialTranslations = {
    ...
    en: {
      steps: {
        intro:
          "Welcome to Term Matching Game! In this game, you need to match the terms on the left (in German) with the correct definitions on the right (in your chosen language).",
        regionSelect:
    ...
    de: {
      steps: {
        intro:
          "Willkommen beim Term Matching Game! In diesem Spiel müssen Sie die Begriffe links (auf Deutsch) den richtigen Definitionen rechts (in der von Ihnen gewählten Sprache) zuordnen.",
        regionSelect:
    ...
    uk: {
      steps: {
        intro:
          "Ласкаво просимо до Term Matching Game! У цій грі вам потрібно зіставити терміни зліва (німецькою) з правильними визначеннями справа (у вибраній вами мові).",
        regionSelect:
    ...
    ru: {
      steps: {
        intro:
          "Добро пожаловать в Term Matching Game! В этой игре вам нужно сопоставить термины слева (на немецком) с правильными определениями справа (на выбранном вами языке).",
        regionSelect:
    ...
    pl: {
      steps: {
        intro:
          "Witamy w Term Matching Game! W tej grze musisz dopasować terminy po lewej stronie (niemieckim) do odpowiednich definicji po prawej (w wybranym przez Ciebie języku).",
        regionSelect:
    ...
    ar: {
      steps: {
        intro:
          "مرحبًا بك في Term Matching Game! في هذه اللعبة، يتعين عليك مطابقة المصطلحات الموجودة على اليسار (بالألمانية) مع التعاريف الصحيحة على اليمين (باللغة التي تختارها).",
        regionSelect:
    ...
    tr: {
      steps: {
        intro:
          "Term Matching Game'e hoş geldiniz! Bu oyunda, soldaki terimleri (Almanca) sağdaki doğru tanımlarla eşleştirmeniz gerekmektedir (seçtiğiniz dilde).",
        regionSelect:
    ...
    fr: {
      steps: {
        intro:
          "Bienvenue dans Term Matching Game ! Dans ce jeu, vous devez associer les termes à gauche (en allemand) aux définitions correctes à droite (dans la langue de votre choix).",
        regionSelect:
